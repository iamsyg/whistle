// frontend/hooks/classroom/fetchClassroomProfile/useFetchNonEmailClassroomProfile.ts
...
export function useFetchNonEmailClassroomProfile(classroom_chat_id: string) {
    ...
    const fetchClassroomProfile = useCallback(async () => {
        ...
        try {
            ...
            console.log('Fetching non-email classroom profile for chat_id:', classroom_chat_id);

            const res = await fetch(
                `${process.env.EXPO_PUBLIC_BACKEND_URL}/classroom/profile/non-email?classroom_chat_id=${classroom_chat_id}`,
                {
                    method: "GET",
                    headers: {
            ...
            console.log('Non-email classroom profile response:', raw);
